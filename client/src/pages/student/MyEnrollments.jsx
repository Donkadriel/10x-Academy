...
const MyEnrollments = () => {
  ...
  return (
    <>
      <div className="md:px-36 pt-30 px-8 ">
          ...
          <tbody className="text-gray-700">
            {enrolledCourses.map((course, index) => (
              <tr key={index} className="border-b border-gray-500/20">
                <td className="md:px-4 pl-5 md:pl-4 py-3 flex items-center space-x-3">
                  <img
                    src={course.courseThumbnail}
                    alt=""
                    className="w-14 smw-24 md:w-28"
                  />
                ...
                <td className="px-4 py-3 max-sm:text-right">
                  <button className="px-3 sm:px-5 text-white py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs" onClick={()=> navigate("/player/" + course._id)}>
                    {
